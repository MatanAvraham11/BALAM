...
יש שני סוגי שגיאות אפשריות בפלט:

1. **תווים שנדבקים מהשוליים** — חתוך הקרופ עלול להכניס תוו/שניים מהתא הסמוך (`תאריך הדפסה`). דוגמה: «**ך** חיים קאופמן» (`ך` הוא הסוף של «תאריך»). מטופל אוטומטית: `_buyer_ocr_label_clean` קורא ל־`_strip_short_edge_tokens` שמסיר מילים של אות אחת בקצה (התחלה/סוף). מילות שם של שתי אותיות (`בן`, `בר`, `אל`) **לא** מוסרות.

2. **בלבול אותיות בתוך השם** — מנועי OCR מבלבלים אותיות דומות בפיקסלים: **י↔ו**, **י↔ף סופית**, ולפעמים מכפילים תווים (`שלאום` במקום `שלאם`, `סורנני` במקום `סורני`). אין תיקון אוטומטי: כל היוריסטיקה תשבור שמות אמיתיים (למשל «יוסף» אמיתי יהפוך ל־«יוסי»). אפשרויות שיפור מציאותיות:
